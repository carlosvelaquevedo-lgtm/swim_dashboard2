...
class SwimAnalyzer:
    ...
    def _initialize_detector(self) -> vision.PoseLandmarker:
        ...
        if not os.path.exists(model_path):
            ...
            try:
                ...
                st.markdown("Please download the model manually from [here](https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_heavy/float16/latest/pose_landmarker_heavy.task) and place it in the current directory as 'pose_landmarker_heavy.task'.")
                st.stop()
    ...
    def _smooth_values(self, elbow: float, knee_left: float, 
        ...
        def safe_mean(buf, fallback):
            return statistics.mean(buf) if len(buf) > 0 else fallback
        
        return (
            safe_mean(self.elbow_buffer, elbow),
            safe_mean(self.knee_left_buffer, knee_left),
            safe_mean(self.knee_right_buffer, knee_right),
            safe_mean(self.roll_buffer, roll)
        )
